...
## Postscript (2026-05-03) — what we kept

The proposal above describes a `pole` primitive that would share state
across resonators in a structured way. We didn't end up adding it as a
primitive. The user-space experiments suggested a richer "aether
substrate" framing (a dimensionless field carrying broadband drive
between voices) and that path was explored at length, but ultimately
abandoned: the substrate framing was 100% documentation overhead with
no engine support behind it, and everything it claimed could be done
with ordinary state cells.

What survived into the codebase:

- `patches/sympathy.aither` — the original positive musical finding
  rewritten without substrate framing. A single leaky-integrator state
  cell (`$bus`) acts as a shared excitation bus; three `dho` voices
  read from it, each filtering out its own resonant frequency. This
  is the "function of state" pattern — no special primitive needed.
- `patches/al_muqabala.aither` — the cybernetic stabilisation patch,
  which never depended on a substrate convention.
- The Hilbert-pair primitives (`analytic`, `rotate`, `freq_shift`,
  `cmul`) — useful as engine-level functions on plain state, see
  `COMPOSING.md`.

What was removed:

- `aether.md` — the substrate convention spec; no engine referent.
- All `aether_pair_*`, `aether_muqabala*`, `aether_polyphase*`,
  `polyphase_envelope_fb`, and `muqabala_*` companion patches.
- The substrate exploration write-ups that used to live below this
  line in pole.md (the Apr 28-29 negative findings, synthesis,
  positive aether finding, cybernetic test, and pair-aether tests).

If you need that history, it's in the git log between commits
`50c1b67..0734513`.

The early monopole/multipole exploration above this line is preserved
because the *acoustic intuition* (one frequency × multiple decay rates,
multiple frequencies sharing a substrate, slowly retuning a pole through
a bank) is still useful. Just read "shared state cell" everywhere the
old text said "field" or "aether."
